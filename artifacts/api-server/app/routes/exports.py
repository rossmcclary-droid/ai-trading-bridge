...
async def export_ai_scan(
    ...
    _discovery_substage_ms = {}
    _timing_stage_started = _timing_started
    _timings_ms: dict[str, float] = {}
    _timing_export_started = time.perf_counter()
    _per_symbol_timings: list[dict[str, Any]] = []
    _discovery_substage_ms: dict[str, float] = {}
    _mega_timings: dict[str, float] = {}
    ...
    def _finish_stage(name: str) -> None:
        ...
        _aliases={"stage_1_account_and_connector":"account_instrument_detail_lookup","stage_2_market_discovery":"other_discovery","stage_3_scan":"history_retrieval","stage_4_analysis":"candidate_analysis","stage_5_payload_assembly":"payload_assembly","stage_6_truth_layer":"truth_layer","stage_7_bootstrap_and_response":"bootstrap_status"}
        if name in _aliases: _mega_timings[_aliases[name]]=_timings_ms[name]
    ...
    _quote_stats={"n_tasks":0,"gather_wall_ms":0.0,"task_elapsed_ms":[]}
    if symbols:
        ...
        try:
            ...
            async def _timed_discover_quote(candidate):
                ...
                try:
                    return await _discover_quote(candidate)
                finally:
    ...
    _discovery_parent_ms = round((time.perf_counter() - _discovery_started) * 1000, 3)
    _discovery_substage_ms.setdefault("universe_instrument_acquisition_ms", 0.0)
    _discovery_substage_ms["quote_retrieval_gather_ms"] = _quote_stats.get("gather_wall_ms", 0.0)
    _discovery_substage_ms.setdefault("shortlist_construction_ms", 0.0)
    _discovery_substage_ms.setdefault("filtering_normalization_ms", 0.0)
    _discovery_substage_ms.setdefault("resolver_spec_details_ms", 0.0)
    _discovery_substage_ms.setdefault("waits_retries_delays_ms", 0.0)
    _discovery_substage_ms["other_discovery_ms"] = round(max(0.0, _discovery_parent_ms - sum(_discovery_substage_ms.values())), 3)
    market_discovery["timing_substages_ms"] = dict(_discovery_substage_ms)
    market_discovery["timing_parent_total_ms"] = _discovery_parent_ms
    _shortlist_raw_count = len(requested_symbols)
    _shortlist_unique_symbols = list(dict.fromkeys(requested_symbols))
    _duplicate_symbols = sorted({x for x in requested_symbols if requested_symbols.count(x) > 1})
    market_discovery["shortlist_raw_count"] = _shortlist_raw_count
    market_discovery["shortlist_unique_count"] = len(_shortlist_unique_symbols)
    market_discovery["duplicate_symbols"] = _duplicate_symbols
    ...
    _discovery_substage_ms["shortlist_construction_ms"]=round((time.perf_counter()-_ss0)*1000,3)
    _discovery_substage_ms["account_instrument_resolution_ms"]=round(float(_timings_ms.get("stage_1_account_and_connector",0.0)),3)
    ...
    for symbol in requested_symbols:
        if _symbol_timing is not None:
            _symbol_timing["total_ms"] = round((time.perf_counter() - _symbol_timing_started) * 1000, 3)
            _per_symbol_timings.append(_symbol_timing)
        _symbol_timing = {"symbol": symbol, "total_ms": 0.0, "history_ms": None, "analysis_ms": 0.0, "timeframe_request_count": len(TIMEFRAMES), "retry_count": 0}
        _symbol_timing_started = time.perf_counter()
        ...
        _analysis_started = time.perf_counter()
        analysis = analyze_scan(scan)
        _symbol_timing["analysis_ms"] = round((time.perf_counter() - _analysis_started) * 1000, 3)
        _ht = scan.get("_patch002c_timing", {}) if isinstance(scan, dict) else {}
        _symbol_timing["history_network_ms"] = round(float(_ht.get("history_network_ms",0.0)),3)
        _symbol_timing["history_delay_sleep_ms"] = round(float(_ht.get("history_delay_sleep_ms",0.0)),3)
        _symbol_timing["history_or_analysis_cpu_ms"] = round(float(_ht.get("history_or_analysis_cpu_ms",_symbol_timing.get("analysis_ms",0.0))),3)
    ...
    if _symbol_timing is not None:
        _symbol_timing["total_ms"] = round((time.perf_counter() - _symbol_timing_started) * 1000, 3)
        _per_symbol_timings.append(_symbol_timing)
    _per_symbol_aggregate = {"sum_ms": round(sum(x["total_ms"] for x in _per_symbol_timings), 3), "max_ms": round(max((x["total_ms"] for x in _per_symbol_timings), default=0.0), 3), "slowest_symbol": max(_per_symbol_timings, key=lambda x: x["total_ms"])["symbol"] if _per_symbol_timings else None, "execution_mode": "serial"}
    ...
    _stage_7_body_finished = time.perf_counter()
    _timings_ms["stage_7_body_ms"] = round((_stage_7_body_finished - _timing_stage_started) * 1000, 3)
    _finish_stage("stage_7_bootstrap_and_response")
    _stage_7_finished = time.perf_counter()
    export_payload["diagnostics"] = {
        "patch002_timings_ms": dict(_timings_ms),
        "timing_clock": "perf_counter",
        "patch002b_timings": {"export_ai_scan_total_ms": round((time.perf_counter() - _timing_export_started) * 1000, 3), "market_discovery_parent_total_ms": _discovery_parent_ms, "market_discovery_substages_ms": dict(_discovery_substage_ms), "per_symbol_timings": _per_symbol_timings, "per_symbol_aggregate": _per_symbol_aggregate, "shortlist_raw_count": _shortlist_raw_count, "shortlist_unique_count": len(_shortlist_unique_symbols), "duplicate_symbols": _duplicate_symbols, "duplicate_processing": {x: sum(1 for y in _per_symbol_timings if y["symbol"] == x) for x in _duplicate_symbols}},
    }

    from app.services.export_timings import build_patch002c, build_patch002d, build_patch002dj
    export_payload.setdefault("diagnostics", {})["patch002c_timings"] = build_patch002c(
        scan=round((time.perf_counter()-_timing_started)*1000,3), stages=_timings_ms, discovery=_discovery_substage_ms,
        qn=locals().get("_patch002c_quote_gather",{}).get("n_tasks",0), qwall=locals().get("_patch002c_quote_gather",{}).get("gather_wall_ms",0), qfail=locals().get("_patch002c_quote_gather",{}).get("failures",0),
        rows=_per_symbol_timings, raw=len(requested_symbols), unique=len(set(requested_symbols)), dups=sorted({x for x in requested_symbols if requested_symbols.count(x)>1}),
    )

    _diag=export_payload.setdefault("diagnostics", {})
    _quote_stats["wall_ms_invalid_legacy"] = None; _quote_stats["slowest_ms_invalid_legacy"] = None
    _quote_stats["consistency_note"] = (
        "gather_wall_ms is true asyncio.gather elapsed; max(task_elapsed_ms) is "
        "the slowest individual task; Semaphore(6) is the concurrency limit; "
        "these are different quantities."
    )
    _diag["patch002d_timings"]=build_patch002d(wall_created_to_ready_ms=round((time.perf_counter()-_timing_export_started)*1000,3),stages=_timings_ms,mega_bucket_split=[{"name":str(k),"ms":v} for k,v in _mega_timings.items()],other_discovery_split=[{"name":str(k),"ms":v} for k,v in _discovery_substage_ms.items()],quote=locals().get("_quote_stats",locals().get("_patch002c_quote_gather",{})),history=locals().get("_patch002d_history",{}),rows=_per_symbol_timings,duplicates=locals().get("_duplicate_symbols",[]),events=[])
    ...
    _response_json_started = time.perf_counter()
    ...
    _response_json_finished = time.perf_counter()
    _timings_ms["response_json_dumps_ms"] = round((_response_json_finished - _response_json_started) * 1000, 3)
    _response_return_started = time.perf_counter()
    _timings_ms["response_return_path_ms"] = round((_response_return_started - _stage_7_finished) * 1000, 3)
    return Response(
...
_EXPORT_TASK_IDS: set[str] = set()
_EXPORT_RECONCILED = False


def _reconcile_export_jobs() -> None:
    global _EXPORT_RECONCILED
    if not _EXPORT_RECONCILED:
        export_job_store.reconcile_orphans(_EXPORT_TASK_IDS)
        _EXPORT_RECONCILED = True


def _load_export_job(job_id: str, *, include_content: bool = False) -> dict[str, Any] | None:
    _reconcile_export_jobs()
    job = export_job_store.get_job(job_id, include_content=include_content)
    if job is not None:
...
async def _run_ai_scan_export_job(
    ...
    from app.services.export_timings import build_patch002dj

    job = _EXPORT_JOBS[job_id]
    _task_started_perf = time.perf_counter()
    _patch002dj_clock = job.setdefault("_patch002dj_clock", {})
    _patch002dj_clock["task_start"] = _task_started_perf
    _created_perf = job["created_perf"]
    job["job_creation_to_task_start_ms"] = round((_task_started_perf - _created_perf) * 1000, 3)
    job["async_schedule_wait_ms"] = job["job_creation_to_task_start_ms"]
    _wrapper_call_started = time.perf_counter()
    _patch002dj_clock["export_start"] = _wrapper_call_started
    try:
        response = await export_ai_scan(
            nickname=nickname,
            symbols=symbols,
        )
        _export_finished_perf = time.perf_counter()
        _patch002dj_clock["export_done"] = _export_finished_perf
        job["wrapper_pre_export_ms"] = round((_wrapper_call_started - _task_started_perf) * 1000, 3)
        ...
        job["payload_assembly_serialization_ms"] = round((time.perf_counter() - _payload_assembly_started) * 1000, 3)
        job["post_export_write_prepare_ms"] = round((time.perf_counter() - _post_export_prepare_started) * 1000, 3)
        ...
        job["job_state_finalization_ms"] = round((time.perf_counter() - _state_finalization_started) * 1000, 3)
        job["total_created_to_ready_ms"] = round((time.perf_counter() - _created_perf) * 1000, 3)
        if _job_payload is not None:
            _job_diag.update({k: job[k] for k in ("job_creation_to_task_start_ms", "async_schedule_wait_ms", "wrapper_pre_export_ms", "export_ai_scan_total_ms", "post_export_write_prepare_ms", "job_state_finalization_ms", "total_created_to_ready_ms")})
            job["content"] = json.dumps(_job_payload, indent=2, ensure_ascii=False).encode("utf-8")
        _durable_save_started = time.perf_counter()
        export_job_store.save_job(job)
        job["durable_result_write_ms"] = round((time.perf_counter() - _durable_save_started) * 1000, 3)
        _patch002dj_clock["ready_visible"] = time.perf_counter()
        if _job_payload is not None:
...
async def start_ai_scan_export(
    ...
    _patch002dj_queued_perf = time.perf_counter()
    _EXPORT_JOBS[job_id]["_patch002dj_clock"] = {"created": _EXPORT_JOBS[job_id]["created_perf"], "queued": _patch002dj_queued_perf}
    export_job_store.save_job(_EXPORT_JOBS[job_id])
    ...
    _EXPORT_TASK_IDS.add(job_id)
    def _forget_export_task(done_task: asyncio.Task[Any]) -> None:
        _EXPORT_TASKS.discard(done_task)
        _EXPORT_TASK_IDS.discard(job_id)
    task.add_done_callback(_forget_export_task)
...
async def download_ai_scan_export_job(
    ...
    job = _load_export_job(job_id, include_content=True)
