...
def build_patch002d(**k):
    ...
    hn=_num(h.get("history_network_ms")); hd=_num(h.get("history_delay_sleep_ms")); hs=_num(h.get("history_success_delay_sleep_ms")); hp=None if h.get("history_processing_ms") is None else _num(h.get("history_processing_ms"))
...
def build_patch002dj(job, *, export_payload=None):
    ...
    top_sources = {
        ...
        "account_and_connector": measured("stage_1_account_and_connector", "account_instrument_resolution_ms", "account_instrument_resolution"),
    ...
    if total is None and isinstance(job, dict):
        total = number(job.get("created_to_ready_ms"))
    if total is None and isinstance(job, dict):
        total = number(job.get("total_created_to_ready_ms"))
    if total is None and isinstance(job, dict):
        total = number(job.get("ready_visible_ms"))
    if total is None and isinstance(clock, dict):
        created = number(clock.get("created"))
        ready = number(clock.get("ready_visible"))
        if created is not None and ready is not None:
            total = round((ready - created) * 1000, 3)
    residual = round(max(0.0, total - top_sum), 3) if total is not None else None
