...
export default function App() {
  ...
  const [exportingAiScan, setExportingAiScan] = useState(false);
    const [preparedAiScanFile, setPreparedAiScanFile] =
  ...
  async function exportAiScan() {
    ...
    try {
        const startResponse = await fetch(
            "/api/scan/Challenge/export-ai/start",
            {
                method: "POST",
            },
        );
        ...
        if (!startResponse.ok) {
            throw new Error(
                `Start HTTP ${startResponse.status}`,
            );
        }
        ...
        if (!jobId) {
            throw new Error(
                "Export job did not return a job ID.",
            );
        }

        let jobStatus = "BUILDING";
        ...
        while (jobStatus === "BUILDING") {
            ...
            jobStatus = statusPayload?.status;
            jobError = statusPayload?.error ?? null;
        }

        if (jobStatus !== "READY") {
            throw new Error(
                jobError ||
                    `Export job ended with status ${jobStatus}.`,
            );
        }

        const response = await fetch(
            `/api/scan/export-ai/jobs/${jobId}/download`,
        );
...
async function scanChallenge() {
   ...
   async function sharePreparedAiScan() {
       if (!preparedAiScanFile) {
           setError("AI Scan file is not ready.");
           return;
       }
       ...
       try {
           if (typeof navigator.share === "function") {
               try {
                   await navigator.share({
                       files: [preparedAiScanFile],
                       title: "Atlas AI Scan",
                   });

                   setPreparedAiScanFile(null);
                   return;
               } catch (shareError) {
                   if (
                   ...
                   console.warn(
                       "Native sharing failed; using file fallback.",
                       shareError,
                   );
               }
           }

           const url = URL.createObjectURL(preparedAiScanFile);
           const link = document.createElement("a");

           link.href = url;
           link.download = preparedAiScanFile.name;
           link.target = "_blank";
           ...
           setError(
               err instanceof Error
                   ? `AI Scan share failed: ${err.message}`
                   : "AI Scan share failed.",
           );
       }
   }
